...
const SERVICE_ID = 'cloud-collab-admin-review-preview';
const API_VERSION = '2026-07-19-stage5b';
...
function success(data) {
  if (!isAdminReviewProjectionSafe(data)) {
    throw new AdminReviewError('ADMIN_REVIEW_UNSAFE_PROJECTION', '管理员审核投影包含禁止字段', 500);
  }
  return jsonResponse({
    ok: true,
    ...
    data: {
      ...data,
      capabilities: ADMIN_REVIEW_CAPABILITIES,
    },
  });
}
...
function failure(error, { clearSession = false } = {}) {
  ...
  const code = typeof error?.code === 'string' && error.code
    ? error.code
    : 'ADMIN_REVIEW_INTERNAL_ERROR';
  const message = status >= 500
    ? '管理员只读审核暂时不可用'
    : (error?.message || '管理员只读审核请求失败');
  return jsonResponse({
    ok: false,
    serviceId: SERVICE_ID,
    apiVersion: API_VERSION,
...
function methodNotAllowed(method, allow) {
  return jsonResponse({
    ...
    serviceId: SERVICE_ID,
    apiVersion: API_VERSION,
    error: { code: 'METHOD_NOT_ALLOWED', message: `管理员只读审核接口不支持 ${method || 'UNKNOWN'} 方法` },
...
function parseRequestUrl(request) {
  try {
    ...
    throw new AdminReviewError('ADMIN_REVIEW_REQUEST_INVALID', '管理员只读审核请求地址无效', 400);
  }
}

function assertNoQuery(request) {
  if ([...parseRequestUrl(request).searchParams.keys()].length !== 0) {
    throw new AdminReviewError('ADMIN_REVIEW_QUERY_INVALID', '审核队列查询参数无效', 400);
  }
}
...
function readDetailId(request) {
  ...
  if (keys.length !== 1 || keys[0] !== 'id' || values.length !== 1) {
    throw new AdminReviewError('ADMIN_REVIEW_QUERY_INVALID', '审核详情查询参数无效', 400);
  }
  return values[0];
}
...
function authenticateAndConfigure(context, dependencies) {
  ...
  const reviewConfig = readAdminReviewConfig(env);
...
export async function handleAdminReviewQueueRequest(context, dependencies = {}) {
  ...
  try {
    ...
    const listQueue = dependencies.listQueue || listAdminReviewQueue;
...
export async function handleAdminReviewDetailRequest(context, dependencies = {}) {
  const method = requestMethod(context?.request);
  if (method !== 'GET') return methodNotAllowed(method, 'GET');
  try {
    const { env, identity, reviewConfig } = authenticateAndConfigure(context, dependencies);
    ...
    const store = createReviewStore(env, reviewConfig, dependencies);
    const getDetail = dependencies.getDetail || getAdminReviewDetail;

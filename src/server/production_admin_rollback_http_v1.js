...
export function readProductionAdminRollbackConfig(env = {}) {
  ...
  return Object.freeze({
    ...
    rollbackRefSalt: runtime.secrets.CLOUD_ADMIN_ROLLBACK_REF_SALT,
...
function failure(error) {
  ...
  const code = typeof error?.code === 'string' && error.code
    ? error.code
    : 'PRODUCTION_ADMIN_ROLLBACK_INTERNAL_ERROR';
  const message = status >= 500
    ? '正式管理员公共数据回滚暂时不可用'
    : (error?.message || '正式管理员回滚请求失败');
  return jsonResponse({
    ok: false,
    serviceId: SERVICE_ID,
    apiVersion: API_VERSION,
...
async function readJsonBody(request) {
  ...
  if (Number.isFinite(declared) && declared > ADMIN_ROLLBACK_MAX_BODY_BYTES) {
...
export async function handleProductionAdminRollbackListRequest(context, dependencies = {}) {
  ...
  try {
    ...
    return success(identity, rollbackConfig, result);
  } catch (error) {

...
const SERVICE_ID = 'cloud-collab-admin-ordinary-review-mutation-preview';
...
function success(identity, result) {
  const data = {
    ...
    result,
    capabilities: ADMIN_ORDINARY_REVIEW_MUTATION_CAPABILITIES,
  };
  if (!isAdminOrdinaryReviewMutationProjectionSafe(data)) {
    throw new AdminOrdinaryReviewMutationError('ADMIN_ORDINARY_REVIEW_UNSAFE_RESPONSE', '管理员普通共享审核响应包含禁止字段', 500);
  }
...
function failure(error, { clearSession = false } = {}) {
  ...
  const code = typeof error?.code === 'string' && error.code
    ? error.code
    : 'ADMIN_ORDINARY_REVIEW_MUTATION_INTERNAL_ERROR';
  const message = status >= 500
    ? '管理员普通共享审核写入暂时不可用'
    : (error?.message || '管理员普通共享审核写入失败');
  return jsonResponse({
    ok: false,
...
function methodNotAllowed(method) {
  return jsonResponse({
    ...
    serviceId: SERVICE_ID,
    apiVersion: API_VERSION,
    error: { code: 'METHOD_NOT_ALLOWED', message: `管理员普通共享审核写入接口不支持 ${method || 'UNKNOWN'} 方法` },
...
function assertNoQuery(request) {
  ...
  try {
    ...
    throw new AdminOrdinaryReviewMutationError('ADMIN_ORDINARY_REVIEW_REQUEST_INVALID', '管理员普通共享审核请求地址无效', 400);
  }
  if ([...url.searchParams.keys()].length !== 0) {
    throw new AdminOrdinaryReviewMutationError('ADMIN_ORDINARY_REVIEW_QUERY_INVALID', '管理员普通共享审核写入不接受查询参数', 400);
  }
}
...
async function readJsonBody(request) {
  ...
  if (!/^application\/json(?:\s*;|$)/.test(contentType)) {
    throw new AdminOrdinaryReviewMutationError('ADMIN_ORDINARY_REVIEW_CONTENT_TYPE_INVALID', '管理员普通共享审核写入只接受JSON', 415);
  }
  const text = await request.text();
  const bytes = Buffer.byteLength(text, 'utf8');
  if (bytes < 2 || bytes > ADMIN_ORDINARY_REVIEW_MUTATION_MAX_BODY_BYTES) {
    throw new AdminOrdinaryReviewMutationError('ADMIN_ORDINARY_REVIEW_BODY_SIZE_INVALID', '管理员普通共享审核请求大小无效', 413);
  }
  try {
    return JSON.parse(text);
  } catch (_) {
    throw new AdminOrdinaryReviewMutationError('ADMIN_ORDINARY_REVIEW_JSON_INVALID', '管理员普通共享审核JSON无效', 400);
  }
}
...
function authenticateAndConfigure(context, dependencies) {
  ...
  const mutationConfig = readAdminOrdinaryReviewMutationConfig(env);
...
export function handleAdminOrdinaryReviewApproveRequest(context, dependencies = {}) {
  return handleMutation('approve', context, dependencies);
}

export function handleAdminOrdinaryReviewRejectRequest(context, dependencies = {}) {
  return handleMutation('reject', context, dependencies);
}

export function handleAdminOrdinaryReviewEditAndApproveRequest(context, dependencies = {}) {

...
function invalidFlagResponse() {
  return new Response(JSON.stringify({
    ...
    error: { code: 'PRODUCTION_FLAG_INVALID', message: '管理员服务配置无效' },
  }), {
    status: 503,
    headers: {
...
export function dispatchAdminLoginRequest(context, dependencies = {}) {
  return dispatch(context, dependencies, handleProductionAdminLoginRequest, handlePreviewAdminLoginRequest);
}

export function dispatchAdminSessionRequest(context, dependencies = {}) {
  return dispatch(context, dependencies, handleProductionAdminSessionRequest, handlePreviewAdminSessionRequest);
}

export function dispatchAdminLogoutRequest(context, dependencies = {}) {
  return dispatch(context, dependencies, handleProductionAdminLogoutRequest, handlePreviewAdminLogoutRequest);
}

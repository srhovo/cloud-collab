...
export function readProductionAdminAuthConfig(env = {}) {
  ...
  if (runtime.mode !== 'production' || runtime.flags.admin !== true) {
    throw new AdminAuthError('PRODUCTION_ADMIN_DISABLED', '正式管理员身份能力未开启', 503);
  }
...
function parseBase64UrlJson(segment, label) {
  ...
  let bytes;
  try { bytes = Buffer.from(segment, 'base64url'); }
  catch (_) { throw new AdminAuthError('ADMIN_SESSION_INVALID', `${label}无效`, 401); }
...
export function createProductionAdminSessionToken({
  ...
  if (config?.mode !== 'production' || config?.issuer !== PRODUCTION_ADMIN_ISSUER) {
  ...
  return Object.freeze({ token: `${signingInput}.${signature}`, expiresAt: expiresAt * 1000 });
}
...
export function verifyProductionAdminSessionToken(token, config, { now = Date.now() } = {}) {
  ...
  try { suppliedSignature = Buffer.from(signatureSegment, 'base64url'); }
  catch (_) { throw new AdminAuthError('ADMIN_SESSION_INVALID', '管理员会话签名无效', 401); }
  ...
  if (!valid) throw new AdminAuthError('ADMIN_SESSION_INVALID', '管理员会话声明无效', 401);
...
export function productionAdminLoginRateKey({ username, clientAddress, salt, now = Date.now() } = {}) {
  if (!Number.isSafeInteger(now) || now <= 0) {
  ...
  const subjectHash = createHmac('sha256', Buffer.from(String(salt || ''), 'utf8'))
    .update(`${normalizeRateSubject(username, 'invalid-username')}\u0000${normalizeRateSubject(clientAddress, 'unknown-client')}`, 'utf8')
    .digest('base64url');
...
export async function consumeProductionAdminLoginRate({ store, username, clientAddress, salt, now = Date.now() } = {}) {

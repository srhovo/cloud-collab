...
function invalidFlagResponse() {
  return new Response(JSON.stringify({
    ok: false,
    serviceId: 'cloud-collab-admin-sensitive-dispatch',
    apiVersion: '2026-07-21-stage7t',
    error: { code: 'PRODUCTION_FLAG_INVALID', message: '敏感审核服务配置无效' },
  }), {
    status: 503,
    headers: {
...
export function dispatchSensitiveReviewListRequest(context, dependencies = {}) {
  return dispatch(context, dependencies, handleProductionSensitiveReviewListRequest, handlePreviewList);
}

export function dispatchSensitiveReviewDetailRequest(context, dependencies = {}) {
  return dispatch(context, dependencies, handleProductionSensitiveReviewDetailRequest, handlePreviewDetail);
}

export function dispatchSensitiveReviewApproveRequest(context, dependencies = {}) {
  return dispatch(context, dependencies, handleProductionSensitiveReviewApproveRequest, handlePreviewApprove);
}

export function dispatchSensitiveReviewRejectRequest(context, dependencies = {}) {
  return dispatch(context, dependencies, handleProductionSensitiveReviewRejectRequest, handlePreviewReject);
}

export function dispatchSensitiveReviewEditAndApproveRequest(context, dependencies = {}) {
  return dispatch(context, dependencies, handleProductionSensitiveReviewEditAndApproveRequest, handlePreviewEditAndApprove);
}

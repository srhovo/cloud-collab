...
function assertDependencies(flags) {
  const childEnabled = flags.readSync || flags.ordinarySubmission || flags.autoApproval
    ...
    || flags.migrationExport || flags.admin;
  ...
  if (flags.migrationExport && (!flags.readSync || !flags.admin)) {
    throw new ProductionRuntimeConfigError('PRODUCTION_ROLLOUT_ORDER_INVALID', '迁移导出必须同时具备只读同步和管理员身份能力');
  }
...
export function readProductionRuntimeConfig(env = {}) {
  const flags = Object.freeze({
    ...
    migrationExport: readCompatibleFlag(env, 'CLOUD_PRODUCTION_EXPORT_ENABLED'),
  ...
  const requiredSecrets = Object.freeze({
    ...
    CLOUD_ADMIN_EXPORT_AUDIT_SALT: flags.adminReview || flags.migrationExport,
  });
  const secrets = {};

...
function actorTagFor(identity) {
  ...
  if (!username) {
    throw new AdminRollbackError('ADMIN_ROLLBACK_ACTOR_INVALID', '管理员身份无效', 401);
  }
...
function normalizeEvidence(value, label) {
  ...
  if (deviceIds.length < 1
      ...
      || deviceIds.length !== submissionIds.length
    ...
    throw new AdminRollbackError(
      'ADMIN_ROLLBACK_EVENT_EVIDENCE_INVALID',
      `${label}事件批准证据无效`,
      503,
    );
  }
...
function normalizeInternalEvent(event, config, label = '公共') {
  if (!isPlainObject(config)
      ...
      || !config.groupId
      || !config.libraryId) {
    throw new AdminRollbackError('ADMIN_ROLLBACK_SCOPE_INVALID', '回滚作用域配置无效', 503);
  }
  if (!isPlainObject(event)
      || !Number.isSafeInteger(event.version)
      || event.version < 1
      || event.groupId !== config.groupId
      || event.libraryId !== config.libraryId
...
async function collectHistories({ store, config } = {}) {
  ...
  if (events.length > MAX_ADMIN_ROLLBACK_EVENT_OBJECTS) {
    throw new AdminRollbackError(
      'ADMIN_ROLLBACK_EVENT_LIMIT_EXCEEDED',
      '公共事件数量超过回滚安全上限',
      409,
    );
  }
...
export async function listAdminRollbackCandidates({ store, config } = {}) {
  ...
  if (candidates.length > MAX_ADMIN_ROLLBACK_CANDIDATES) {
    throw new AdminRollbackError(
      'ADMIN_ROLLBACK_CANDIDATE_LIMIT_EXCEEDED',
      '回滚候选数量超过安全上限',
      409,
    );
  }
...
function auditKey(auditId, occurredAt) {
  ...
  return normalizeBlobKey(
    `audit/${date.getUTCFullYear()}/${String(date.getUTCMonth() + 1).padStart(2, '0')}/${auditId}.json`,
  );
}
...
function buildReplaySubmission(target) {
  ...
  try {
    ...
    throw new AdminRollbackError(
      'ADMIN_ROLLBACK_REPLAY_SUBMISSION_INVALID',
      '上一批准值无法构造回滚补偿事件',
      503,
      null,
      error,
...
function assertDecision(value, config, expected = {}) {
  ...
  if (targetSubmission.groupId !== config.groupId
      ...
      || value.rollbackRef !== rollbackRefFromParts(
        config,
        {
...
function assertRequestIndex(value, expectedToken = null) {
  assertExactKeys(
    ...
    'ADMIN_ROLLBACK_REQUEST_INDEX_INVALID',
    '回滚请求索引结构无效',
    503,
  );
  if (value.schemaVersion !== ADMIN_ROLLBACK_SCHEMA_VERSION
...
async function claimRequestIndex(store, key, proposed) {
  try {
    ...
    if (existing.requestHash !== proposed.requestHash
        || existing.rollbackId !== proposed.rollbackId) {
      throw new AdminRollbackError(
...
function assertCompletion(value, decision) {
  assertExactKeys(
    ...
    'ADMIN_ROLLBACK_COMPLETION_INVALID',
    '回滚完成记录结构无效',
    503,
  );
  if (value.schemaVersion !== ADMIN_ROLLBACK_SCHEMA_VERSION
...
async function executeDecision({ store, config, decision }) {
  ...
  if (published.event.businessKey !== decision.businessKey
    ...
    throw new AdminRollbackError(
      'ADMIN_ROLLBACK_EVENT_LINK_INVALID',
      '回滚补偿事件与决策不一致',
      503,
    );
  }
  const record = published.snapshot.records.find(
    item => item.businessKey === decision.businessKey,
  );
  if (!record
      || record.approvedVersion !== published.event.version
      || record.contentHash !== decision.restore.contentHash
      || record.payload.unitPrice !== decision.restore.unitPrice) {
    throw new AdminRollbackError(
      'ADMIN_ROLLBACK_SNAPSHOT_MISMATCH',
      '回滚后快照未恢复到上一批准值',
      503,
    );
  }
  ...
  await putImmutableExact(
    store,
    completionKey(config, decision.rollbackId),
    completion,
...
export async function executeAdminRollback({
  ...
  assertSafeTime(now);
  if (config?.previewEnabled !== true && config?.productionEnabled !== true) {
    throw new AdminRollbackError('ADMIN_ROLLBACK_DISABLED', '管理员回滚能力未开启', 503);
  }
  ...
  const requestIndex = await claimRequestIndex(
    store,
    requestIndexKey(config, requestToken),
    Object.freeze({
      schemaVersion: ADMIN_ROLLBACK_SCHEMA_VERSION,
      requestToken,
      requestHash,
  ...
  if (decision) {
    ...
    const target = await resolveCandidateByRef({
      store,
      config,
  ...
  const existingCompletion = await getJSONStrong(
    store,
    completionKey(config, rollbackId),
  );
  if (existingCompletion) {
    return projectCompletion(
      decision,
      assertCompletion(existingCompletion, decision),
      true,
    );
  }

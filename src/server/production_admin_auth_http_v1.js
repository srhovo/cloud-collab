...
function success(data, options = {}) {
  return response({ ok: true, serviceId: SERVICE_ID, apiVersion: API_VERSION, data }, options);
}
...
function failure(error, extraHeaders = {}) {
  ...
  }, { status, extraHeaders: { ...retryAfter, ...extraHeaders } });
}
...
function methodNotAllowed(request, allow) {
  return response({
    ...
    serviceId: SERVICE_ID,
    apiVersion: API_VERSION,
    error: { code: 'METHOD_NOT_ALLOWED', message: `管理员接口不支持${method(request)}方法` },
...
async function readLoginBody(request) {
  ...
  if (Number.isFinite(declared) && declared > MAX_LOGIN_BODY_BYTES) {
    throw new ProductionAdminAuthError('REQUEST_BODY_TOO_LARGE', '管理员登录请求体过大', 413);
  }
  const text = await request.text();
  if (!text || Buffer.byteLength(text, 'utf8') > MAX_LOGIN_BODY_BYTES) {
    throw new ProductionAdminAuthError('INVALID_LOGIN_BODY', '管理员登录请求体无效', text ? 413 : 400);
  }
  try { return JSON.parse(text); }
  catch (_) { throw new ProductionAdminAuthError('INVALID_JSON_BODY', '管理员登录请求体不是有效JSON', 400); }
}

function requestSubject(request, username) {
  return [
    String(username || '').trim().toLowerCase(),
    String(request?.headers?.get?.('cf-connecting-ip') || request?.headers?.get?.('x-forwarded-for') || 'unknown').split(',')[0].trim(),
  ].join('|');
}
...
export async function requireProductionAdminSession(context, dependencies = {}) {
  ...
  const session = verifyProductionAdminSessionToken(token, config, dependencies.now?.() || Date.now());
  await consumeProductionAdminSessionRateLimit({
    store,
    config,
...
export async function handleProductionAdminLoginRequest(context, dependencies = {}) {
  ...
  try {
    ...
    const store = storeFor(config, dependencies);
    await consumeProductionAdminLoginRateLimit({
      store,
      config,
      subject: requestSubject(request, input?.username),
      now: dependencies.now?.() || Date.now(),
    });
    ...
    const verified = verifyProductionAdminSessionToken(token, config, dependencies.now?.() || Date.now());
    return success(projectProductionAdminSession(verified, config), {
      extraHeaders: { 'Set-Cookie': buildAdminSessionCookie(token, config) },
    });
  } catch (error) {
...
export async function handleProductionAdminSessionRequest(context, dependencies = {}) {
  ...
  try {
    const { config, session } = await requireProductionAdminSession(context, dependencies);
    return success(projectProductionAdminSession(session, config));
  } catch (error) {
    return failure(error);
  }
}
...
export async function handleProductionAdminLogoutRequest(context, dependencies = {}) {
  ...
  try {
    ...
    return success({ loggedOut: true }, {
      extraHeaders: { 'Set-Cookie': clearAdminSessionCookie(config) },
    });
  } catch (error) {
    let config = null;
    try { config = readProductionAdminAuthConfig(context?.env || {}); } catch (_) {}

(function(root, factory) {
  ...
  const WRITE_GATE_HEADER = 'X-Cloud-Collab-Preview-Key';
  ...
  function classifyRemoteError(code, status, remoteRetryable = false) {
    ...
    if (['PREVIEW_WRITE_DISABLED', 'BLOB_STORE_NOT_CONFIGURED', 'PREVIEW_ACCESS_KEY_NOT_CONFIGURED', 'RATE_LIMIT_SALT_NOT_CONFIGURED'].includes(value)) {
      return { category: 'service_disabled', retryable: false };
    }
    ...
    if (status === 403 || ['PREVIEW_ACCESS_DENIED', 'PREVIEW_SCOPE_FORBIDDEN', 'DEVICE_SCOPE_MISMATCH'].includes(value)) {
      return { category: value === 'PREVIEW_ACCESS_DENIED' ? 'write_gate' : 'forbidden', retryable: false };
    }
  ...
  class SubmissionApiClient {
    constructor({ baseUrl = '', fetchImpl = globalThis.fetch, timeoutMs = 10000, writeAccessProvider = null, locationRef = typeof location !== 'undefined' ? location : null } = {}) {
      ...
      this.writeAccessProvider = typeof writeAccessProvider === 'function' ? writeAccessProvider : () => '';
    }

    isConfigured() { return Boolean(this.baseUrl && typeof this.fetchImpl === 'function'); }
    hasWriteAccess() { return Boolean(String(this.writeAccessProvider() || '').trim()); }
    ...
    async request(path, { body, token = null } = {}) {
      ...
      const writeAccess = String(this.writeAccessProvider() || '');
      if (!writeAccess) throw new SubmissionClientError('WRITE_GATE_CLOSED', '候选上传运行时门禁未开启', { category: 'write_gate' });
  ...
  function createConfiguredClient({ documentRef, locationRef, fetchImpl = globalThis.fetch, timeoutMs = 10000, writeAccessProvider = null } = {}) {

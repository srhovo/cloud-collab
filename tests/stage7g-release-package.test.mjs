...
test('阶段7G生成8.2.30候选单文件与最终发布清单', () => {
  const outputDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'stage7g-package-'));
  try {
    ...
    assert.equal(result.candidateFilename, '码单器8.2.30_候选.html');
    assert.equal(result.manifestFilename, '最终发布清单_8.2.30.json');
    assert.equal(result.auditFilename, '发布审计_8.2.30.json');
    ...
    assert.equal(candidate.toString('utf8').includes('<title>码单器8.2.30（公共协作发布候选版）</title>'), true);
    assert.equal(manifest.releaseStatus, 'candidate_packaged_not_promoted');
    assert.equal(manifest.candidate.version, '8.2.30');
...
test('阶段7G最终发布清单对同一构建输入保持确定性', () => {
  const first = fs.mkdtempSync(path.join(os.tmpdir(), 'stage7g-package-a-'));
  const second = fs.mkdtempSync(path.join(os.tmpdir(), 'stage7g-package-b-'));
  try {

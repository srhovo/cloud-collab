...
test('registration request sends no Authorization header', async () => {
  const calls = [];
  ...
  await client.registerDevice({ deviceId: 'dev_x', nickname: null });
...
test('submission token is only in Authorization and never copied into body', async () => {
  ...
  const client = new SubmissionApiClient({ baseUrl: 'https://api.test', fetchImpl: async (url, options) => {
    call = { url, options };
    return response(202, { ok: true, data: { state: 'waiting_confirmation' } });
  } });
...
test('client classifies 429 and 5xx as retryable', async () => {
...
test('dispatcher registers lazily and acknowledges successful queue records', async () => {
  ...
  const apiClient = {
    ...
    async registerDevice() { calls.push('register'); return { deviceId: 'dev_x', deviceToken: 'token', issuedAt: 1, expiresAt: 9999999999999, tokenVersion: 1 }; },
    async submit(token, submission) { calls.push(['submit', token, submission.submissionId]); return { state: 'waiting_confirmation' }; },
  };
  const dispatcher = new SubmissionDispatcher({ apiClient, ...s, now: () => 2 });
  const result = await dispatcher.flush();
  assert.deepEqual(calls, ['register', ['submit', 'token', 'sub_1']]);
...
test('dispatcher retries transient errors and blocks permanent validation errors', async () => {
  ...
  const apiClient = {
    ...
    async submit(token, submission) {
      ...
      throw new SubmissionClientError('INVALID_SUBMISSION', 'bad', { status: 400, retryable: false });
    },
  };
  const dispatcher = new SubmissionDispatcher({ apiClient, ...s, now: () => 2 });
  const result = await dispatcher.flush();

...
FILES = {
    ...
    '/tools/production-secret-generator-config.js': ('application/javascript; charset=utf-8', 'production-secret-generator-config.js'),
...
def main() -> None:
    ...
    def serve(route) -> None:
        ...
        route.fulfill(
            ...
            body=(TOOL / filename).read_text(encoding='utf-8'),
        )
    ...
    with sync_playwright() as playwright:
        ...
        page.click('#generate')
        values = page.locator('#valueRows input').evaluate_all('(nodes) => nodes.map(node => node.value)')
        assert len(values) == 8
        assert len(set(values)) == 8
        assert all(len(value) == 64 for value in values)
        assert all(value.replace('-', '').replace('_', '').isalnum() for value in values)
        ...
        assert 'CLOUD_PRODUCTION_PUBLIC_ORIGIN=' in block
        assert 'CLOUD_ADMIN_PUBLIC_ORIGIN=' in block

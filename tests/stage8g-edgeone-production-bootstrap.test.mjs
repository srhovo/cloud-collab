...
test('默认模式只生成10项阶段8H零写入初始化计划', () => {
  const report = 'dist/stage8h-test-bootstrap-plan.json';
  fs.rmSync(path.join(root, report), { force: true });
  const result = run([], { BOOTSTRAP_REPORT_PATH: report });
  assert.equal(result.status, 0, result.stderr);

  const payload = JSON.parse(result.stdout);
  assert.equal(payload.stage, '8H');
...
test('计划模式即使存在三项凭据也不访问Blob或输出凭据', () => {
...
test('真实执行只接受手动事件、main分支和审批环境', () => {
  ...
  const missingEnvironment = run(['--execute'], executionEnv({ BOOTSTRAP_APPROVAL_ENVIRONMENT: '' }));
  assert.notEqual(missingEnvironment.status, 0);
  assert.match(missingEnvironment.stderr, /BOOTSTRAP_APPROVAL_ENVIRONMENT_INVALID/u);
});

test('真实执行要求双确认和独立随机解锁值', () => {
...
test('SDK失败信息同时脱敏Token、项目ID和执行解锁值', () => {
  const projectId = 'pages-secretproject123';
  const safe = sanitizeFailureMessage(
    `request failed Authorization: Bearer ${token}; token=${encodeURIComponent(token)}; project=${projectId}; execution_unlock=${unlock}`,
    { token, projectId, executionUnlock: unlock },
  );
  assert.doesNotMatch(safe, new RegExp(token, 'u'));
  assert.doesNotMatch(safe, new RegExp(encodeURIComponent(token), 'u'));
  assert.doesNotMatch(safe, new RegExp(projectId, 'u'));
  assert.doesNotMatch(safe, new RegExp(unlock, 'u'));
  assert.match(safe, /\[REDACTED\]/u);
});

test('初始化器使用官方外部SDK强一致参数且保持生产能力关闭', () => {
...
test('手动工作流要求main、审批环境、双文本确认和随机解锁Secret', () => {

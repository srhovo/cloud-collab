...
test('阶段8E交接报告已由阶段8J收敛到单项目双域名且保持零启用', () => {
  ...
  assert.equal(report.revisedAtStage, '8J');
  assert.equal(report.status, 'handoff_ready_single_project_two_domains_waiting_owner_domain_status');
  ...
  assert.equal(report.manualActions.slice(1).every(item => item.requiredNow === false), true);
  assert.deepEqual(report.optionalPreDomainActions, []);
  assert.equal(report.activationBlockers.includes('cross_project_blob_architecture_unresolved'), false);
  assert.equal(report.activationBlockers.includes('domain_status_unconfirmed'), true);
...
test('交接输出只使用一个EdgeOne项目且不需要运行时账户令牌', () => {
  const files = [

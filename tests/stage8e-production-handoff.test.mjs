...
test('阶段8E交接报告已由阶段8L收敛到域名确认与免备案首发路线', () => {
  ...
  assert.equal(report.revisedAtStage, '8L');
  assert.equal(report.status, 'handoff_ready_domain_verified_waiting_edgeone_region_check');
  ...
  assert.equal(report.manualActions[0].requiredNow, false);
  assert.equal(report.manualActions[1].requiredNow, true);
  assert.equal(report.manualActions.slice(2).every(item => item.requiredNow === false), true);
  ...
  assert.equal(report.activationBlockers.includes('domain_ownership_unconfirmed'), false);
...
test('交接输出只使用一个EdgeOne项目且明确当前不购买备案云资源', () => {
  const files = [

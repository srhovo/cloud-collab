...
import { acceptProductionOrdinarySubmission } from '../src/server/production_ordinary_runtime_v1.js';
...
const ACCESS_KEY = 'stage7q-production-client-access-key-0123456789';
const RATE_SALT = 'stage7q-production-rate-limit-salt-0123456789';
...
const SUBMISSIONS = Object.freeze([
  ...
  'sub_01JABCDEF0123456789XYZABCH',
  'sub_01JABCDEF0123456789XYZABCJ',
]);
...
function env(overrides = {}) {
  return {
    ...
    CLOUD_PRODUCTION_AUTO_APPROVAL_HANDLER_ENABLED: '1',
...
function baseEnvelope({ deviceId, submissionId, dataType, payload, bossId = null }) {
  return {
    schemaVersion: 1,
...
function makeSubmission({ deviceId, submissionId, dataType, payload }) {
  const raw = baseEnvelope({ deviceId, submissionId, dataType, payload });
  const hashes = dataType === 'exact_price'
    ? computeSubmissionHashes(raw)
    : computeOrdinarySubmissionHashes(raw);
...
test('自动审核关闭时三类普通候选都只入队且不修改公共库', async () => {
  const cases = [
    makeSubmission({ deviceId: DEVICES[0], submissionId: SUBMISSIONS[0], dataType: 'exact_price', payload: { serviceName: '测试服务', settleType: 'round', unitPrice: 100 } }),
    makeSubmission({ deviceId: DEVICES[1], submissionId: SUBMISSIONS[1], dataType: 'playable_name', payload: { name: '下雪' } }),
    makeSubmission({ deviceId: DEVICES[2], submissionId: SUBMISSIONS[2], dataType: 'boss_profile', payload: { bossName: '测试老板', paiDan: '直属A', discount: 0.9 } }),
  ];
  for (const submission of cases) {
    ...
    assert.equal(result.productionAutoApprovalEnabled, false);
...
test('两台设备提交相同陪玩名字时自动批准并生成公共版本', async () => {
  const store = new MemoryStore();
  ...
  assert.equal(waiting.reviewStatus, 'waiting_confirmation');
  assert.equal(waiting.publicMutationApplied, false);
  assert.equal(waiting.publicVersion, 0);

  const approved = await submit({ store, submission: second, runtimeEnv: enabled, now: NOW + 6000 });
  assert.equal(approved.productionAutoApprovalEnabled, true);
  assert.equal(approved.reviewStatus, 'auto_approved');
  ...
  const snapshot = await buildOrdinaryPublicSnapshot({ store, groupId: 'group_see', libraryId: 'lib_see_cz', now: NOW + 6001 });
...
test('相同业务键的冲突普通候选进入人工审核且不改变公共版本', async () => {
  const store = new MemoryStore();
  const enabled = env({ CLOUD_PRODUCTION_AUTO_APPROVAL_ENABLED: '1' });
  const first = makeSubmission({ deviceId: DEVICES[0], submissionId: SUBMISSIONS[0], dataType: 'playable_name', payload: { name: '下雪' } });
  const conflict = makeSubmission({ deviceId: DEVICES[1], submissionId: SUBMISSIONS[1], dataType: 'playable_name', payload: { name: '下雪2' } });
  const bossA = makeSubmission({ deviceId: DEVICES[0], submissionId: SUBMISSIONS[2], dataType: 'boss_profile', payload: { bossName: '同一老板', paiDan: '直属A', discount: 0.9 } });
  const bossB = makeSubmission({ deviceId: DEVICES[1], submissionId: SUBMISSIONS[3], dataType: 'boss_profile', payload: { bossName: '同一老板', paiDan: '直属A', discount: 0.88 } });
  assert.notEqual(first.businessKey, conflict.businessKey);
  assert.equal(bossA.businessKey, bossB.businessKey);
  await submit({ store, submission: bossA, runtimeEnv: enabled, now: NOW });
  const result = await submit({ store, submission: bossB, runtimeEnv: enabled, now: NOW + 6000 });
  assert.equal(result.reviewStatus, 'pending_review');
  assert.equal(result.publicMutationApplied, false);
  assert.equal(result.publicVersion, 0);
});

test('错误作用域和Authorization设备不一致都失败关闭', async () => {
  const store = new MemoryStore();
...
test('HTTP路由在自动审核开启且处理器就绪时调用统一普通处理器', async () => {
  ...
  const response = await handleProductionSubmissionCreateRequest({
    ...
    acceptProduction: async input => {
      ...
      return {
        ...
        duplicate: false,
        productionAutoApprovalEnabled: true,
...
test('生产提交入口继续使用模式分发器且不直接导入自动审核实现', () => {
  ...
  assert.match(route, /handleSubmissionCreateRequest/u);
  assert.doesNotMatch(route, /ordinary_public_engine_v1|preview_auto_approval/u);
  assert.match(http, /acceptProductionOrdinarySubmission/u);
  assert.match(http, /CLOUD_PRODUCTION_AUTO_APPROVAL_HANDLER_ENABLED/u);
});

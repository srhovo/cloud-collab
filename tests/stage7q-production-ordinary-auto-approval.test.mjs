...
test('相同业务键的冲突普通候选进入人工审核且不改变公共版本', async () => {
  ...
  // playable_name业务键包含名字；用老板资料制造同一老板身份的内容冲突。
...
test('HTTP路由在自动审核开启时调用统一普通处理器而不再返回未实现错误', async () => {
...
test('生产提交入口继续使用模式分发器且不直接导入自动审核实现', () => {
  ...
  assert.doesNotMatch(http, /PRODUCTION_AUTO_APPROVAL_HANDLER_REQUIRED/u);
});

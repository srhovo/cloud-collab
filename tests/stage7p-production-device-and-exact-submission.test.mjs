...
test('精确重放绕过限流，新提交在同一窗口被限制', async () => {
  ...
  assert.equal(retry.publicMutationApplied, false);
...
test('正式注册HTTP只对精确来源授权并投影自动审核开关', async () => {
...
test('正式精确价格HTTP转发Authorization并在自动审核关闭时返回202', async () => {
...
test('自动审核开启时正式路由调用审核处理器而不再配置失败', async () => {
  let storeCalls = 0;
  let handlerCalls = 0;
  const response = await handleProductionSubmissionCreateRequest({
    env: env({ CLOUD_PRODUCTION_AUTO_APPROVAL_ENABLED: '1' }),
    request: post('https://app.example.invalid/api/submissions/create', makeSubmission(), { Authorization: 'Bearer dt_v1_production' }),
  }, {
    createStore() { storeCalls += 1; return new MemoryStore(); },

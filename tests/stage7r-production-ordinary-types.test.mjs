...
import { listValidOrdinaryPublicEvents } from '../src/server/ordinary_public_engine_v1.js';
...
test('普通陪玩名字首次提交等待，第二台设备相同确认发布唯一公共事件', async () => {
  ...
  const first = await acceptProductionSubmission({
    store, authorization: 'Bearer dt_v1_a', rawSubmission: firstSubmission, env: env(), now: NOW,
    authenticate: identityFor(firstSubmission),
  });
  const second = await acceptProductionSubmission({
    store, authorization: 'Bearer dt_v1_b', rawSubmission: secondSubmission, env: env(), now: NOW + 6000,
  ...
  const replay = await acceptProductionSubmission({
    store, authorization: 'Bearer dt_v1_a', rawSubmission: firstSubmission, env: env(), now: NOW + 6000,
    authenticate: identityFor(firstSubmission),
  });
...
test('联系方式与越界作用域在设备认证和限流写入前失败', async () => {
  const contact = playable();
  contact.payload = { name: '微信 wx123456' };
  const outsideScope = playable({ groupId: 'group_other' });
  for (const invalid of [contact, outsideScope]) {
    const store = new MemoryStore();
    let authCalls = 0;
    await assert.rejects(
      () => acceptProductionSubmission({

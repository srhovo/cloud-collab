...
一次PR验证中，下载的`ci.log`出现Node总结`# fail 1`，而Actions步骤元数据却显示成功。该现象可能来自并发合并引用、产物错配或退出码传递异常，因此不能只依赖界面状态判断完整Node门禁。
...
- `tests/stage7v-ci-result-integrity.test.mjs`覆盖成功、真实失败、取消、缺失总结、零测试和嵌套失败样例；
- `.github/workflows/ci.yml`保留命令退出码，并在同一步增加总结校验。

校验器使用最后一个Node总结，使测试自身可以构造失败样例，而不会把内部夹具误判为外层失败。

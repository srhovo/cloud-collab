...
function readExecutionCredentials() {
  ...
  if (eventName !== 'workflow_dispatch') fail('BOOTSTRAP_EVENT_INVALID', '真实初始化只能由GitHub手动工作流触发');
  if (gitRef !== 'refs/heads/main') fail('BOOTSTRAP_BRANCH_INVALID', '真实初始化只能从main分支执行');
  if (approvalEnvironment !== 'production-bootstrap') fail('BOOTSTRAP_APPROVAL_ENVIRONMENT_INVALID', '真实初始化必须经过production-bootstrap环境门禁');
  if (confirmation !== CONFIRMATION) fail('BOOTSTRAP_CONFIRMATION_INVALID', '一次性初始化确认词不匹配');
  if (acknowledgement !== IMPACT_ACKNOWLEDGEMENT) fail('BOOTSTRAP_IMPACT_ACKNOWLEDGEMENT_INVALID', '不可变写入影响确认不匹配');
  if (!/^pages-[A-Za-z0-9_-]{6,}$/u.test(projectId)) fail('EDGEONE_PROJECT_ID_INVALID', 'EDGEONE_PROJECT_ID缺失或格式无效');
  if (Buffer.byteLength(token, 'utf8') < 20) fail('EDGEONE_API_TOKEN_INVALID', 'EDGEONE_API_TOKEN缺失或过短');
  if (Buffer.byteLength(unlock, 'utf8') < 32) fail('BOOTSTRAP_EXECUTION_LOCKED', '初始化执行解锁值缺失或过短');
  if (unlock === token || unlock === projectId || token === projectId) fail('BOOTSTRAP_EXECUTION_VALUE_REUSED', '项目ID、API Token和执行解锁值不得复用');
...
async function main() {
  try {
    ...
    fail(error?.code || 'EDGEONE_PRODUCTION_BOOTSTRAP_FAILED', sanitizeFailureMessage(error?.message || 'EdgeOne生产初始化失败'), 1);
  }
}

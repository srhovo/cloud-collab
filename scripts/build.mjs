...
` constructor(app, stores, apiClient, submissionApi, submissionDispatcher, submissionIdFactory) {
  this.app = app;
  this.stores = stores;
...
` this.priceLibraryStore = new PriceLibraryStore(storage, this.priceMemoryStore);
 ...
 this.cloudCollabSubmissionApi = CloudCollabSubmission.createConfiguredClient({
  ...
  timeoutMs: 8000,
  writeAccessProvider: () => {
   const provider = globalThis.CloudCollabPreviewRuntime?.getWriteAccess;
   return typeof provider === 'function' ? String(provider() || '') : '';
  }
 });

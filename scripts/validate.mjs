...
check('Stage5BC cleanup page exposes only non-sensitive error codes and forgets rejected keys', stage5bcCleanupPage.includes('错误码：${code}') && stage5bcCleanupPage.includes("state.key = '';") && stage5bcCleanupPage.includes('STAGE5BC_CLEANUP_NETWORK_ERROR') && stage5bcCleanupPage.includes('环境变量保存后必须生成并打开新的清理部署'));

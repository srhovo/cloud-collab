...
import { preparePublicCandidate, PUBLIC_CANDIDATE_FILES } from './prepare-public-candidate-v1.mjs';
...
const fail = (code, message, details = null) => { throw new Stage7HRehearsalError(code, message, details); };
...
function assertJsonUtf8(config, source) {
  if (String(headerMap(config, source).get('content-type') || '').toLowerCase() !== 'application/json; charset=utf-8') {
...
export function auditStage7HRepository({ root } = {}) {
  ...
  if (releaseAudit.status !== 'promotion_authorization_required' || releaseAudit.blockers.length !== 0) {
    fail('STAGE7H_RELEASE_NOT_READY', '候选发布证据未闭环', { status: releaseAudit.status, blockers: releaseAudit.blockers });
  }
  ...
  includesAll(pagesWorkflow, [
    'workflow_dispatch:', 'candidate_version:', 'DEPLOY-CANDIDATE-8.2.31',
    'npm ci --ignore-scripts', 'npm run ci', 'npm run public:prepare',
    'path: ./.pages-artifact', 'actions/deploy-pages@v4', 'npm run public:verify',
  ], 'STAGE7H_PAGES_GATE_INCOMPLETE', 'GitHub Pages工作流');
  includesAll(rehearsalWorkflow, [
    'pull_request:', 'npm ci --ignore-scripts', 'npm run release:rehearse',
    '.pages-artifact', '.edgeone-artifact', 'dist/stage7h-release-rehearsal.json', FINAL_RELEASE_MANIFEST,
  ], 'STAGE7H_REHEARSAL_WORKFLOW_INCOMPLETE', '候选发布预演工作流');
  if (rehearsalWorkflow.includes('actions/deploy-pages')) fail('STAGE7H_REHEARSAL_DEPLOYMENT_PRESENT', '候选发布预演工作流不得执行真实部署');
  ...
  if (!cacheControl.includes('max-age=0') || !cacheControl.includes('must-revalidate')
      || String(genericHeaders.get('x-content-type-options') || '').toLowerCase() !== 'nosniff'
      || String(genericHeaders.get('x-frame-options') || '').toLowerCase() !== 'deny'
      || String(genericHeaders.get('referrer-policy') || '').toLowerCase() !== 'no-referrer') {
    fail('STAGE7H_EDGEONE_HEADERS_INVALID', 'EdgeOne候选入口缓存或安全响应头无效');
  }
  assertJsonUtf8(edgeOneConfig, '/build-manifest.json');
  assertJsonUtf8(edgeOneConfig, '/pages-release.json');
  ...
  if (finalManifest.candidate?.version !== CANDIDATE_VERSION
      || finalManifest.candidate?.title !== '码单器8.2.31（公共协作发布候选版）'
      || !/^[a-f0-9]{64}$/.test(String(finalManifest.candidate?.sha256 || ''))
      || !Number.isSafeInteger(finalManifest.candidate?.bytes) || finalManifest.candidate.bytes <= 0
  ...
  includesAll(readme, ['阶段7J', CANDIDATE_VERSION, finalManifest.candidate.sha256, '稳定版8.2.25未晋升', '正式公共写入保持关闭'], 'STAGE7H_README_STALE', 'README');
  includesAll(guide, ['EdgeOne Pages', 'GitHub Pages', 'ICP备案', 'Wi-Fi', '移动数据', '不晋升稳定版'], 'STAGE7H_GUIDE_INCOMPLETE', '大陆访问入口方案');

  return Object.freeze({
    schemaVersion: 1,
    stage: '7J-candidate-release-rehearsal',
    status: 'rehearsal_ready_not_deployed',
    candidate: Object.freeze({ ...finalManifest.candidate }),
    publicArtifactAllowlist: PUBLIC_CANDIDATE_FILES,
    entryStrategy: Object.freeze({ primary: 'edgeone-pages-candidate-preview', backup: 'github-pages-manual-candidate', offline: 'verified-single-html' }),
    ...
    boundaries: Object.freeze({ deploymentPerformed: false, stablePromotionAuthorized: false, stablePromotionPerformed: false, productionWriteEnablementIncluded: false, blobMutationsPerformed: 0 }),
  });
}
...
export function rehearseStage7H({ root, commitSha } = {}) {
  ...
  const pages = preparePublicCandidate({ root: repositoryRoot, outputDirectory: path.join(repositoryRoot, '.pages-artifact'), commitSha: commit, channel: 'github-pages-backup' });
  const edgeOne = preparePublicCandidate({ root: repositoryRoot, outputDirectory: path.join(repositoryRoot, '.edgeone-artifact'), commitSha: commit, channel: 'edgeone-primary' });
  for (const filename of ['index.html', 'build-manifest.json']) {
    if (!fs.readFileSync(path.join(pages.outputDirectory, filename)).equals(fs.readFileSync(path.join(edgeOne.outputDirectory, filename)))) {
  ...
  const report = {
    ...
    sourceCommit: commit,
    artifacts: {
      githubPagesBackup: { directory: '.pages-artifact', files: pages.files, channel: pages.release.channel },
      edgeOnePrimary: { directory: '.edgeone-artifact', files: edgeOne.files, channel: edgeOne.release.channel },
  ...
  fs.writeFileSync(path.join(repositoryRoot, 'dist', 'stage7h-release-rehearsal.json'), `${JSON.stringify(report, null, 2)}\n`, 'utf8');
...
function run() {
  ...
  const report = auditOnly ? auditStage7HRepository({ root }) : rehearseStage7H({ root });
  if (auditOnly) fs.writeFileSync(path.join(root, 'dist', 'stage7h-release-rehearsal.json'), `${JSON.stringify(report, null, 2)}\n`, 'utf8');
